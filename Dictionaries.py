...
''' This one works! '''
home_outcomes['altitude'] = home_outcomes['home_team_id'].map(
    city_alt).fillna(0)

home_outcomes['team_name'] = home_outcomes['home_team_id'].map(
    id_team).fillna('N/A')

home_outcomes['home_win_pct'] = home_outcomes['team_name'].map(
    home_total_pct).fillna(0)
...
date_outcomes = home_outcomes.drop(
    columns=['altitude', 'home_win_pct', 'home_team_id']).copy()
...
toronto_outcomes['long_date'] = pd.to_datetime(
    toronto_outcomes['date_time_GMT'])
...
toronto_short = toronto_outcomes.drop(
    columns=['date_time_GMT', 'long_date']).copy()

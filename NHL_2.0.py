...
def read_clean(csv): 
    ...
    cleaned['altitude'] = cleaned['home_team_id'].map(NHL.city_alt).fillna('Other')
    cleaned['team_name'] = cleaned['home_team_id'].map(NHL.id_team).fillna('Other')

    # add new column to cleaned called 'home_win_pct' -- contains total home win percentage over 5 seasons
    cleaned['home_total_pct'] = cleaned['team_name'].map(NHL.home_total_pct).fillna('N/A')
...
def record_finder(name): 
    ...
    home_wins = record.loc[record['outcome'].isin(['home win REG','home win OT'])].shape[0]

    # determines total home losses (counted as away wins) 
    home_losses = record.loc[record['outcome'].isin(['away win REG', 'away win OT'])].shape[0]
...
condensed_outcomes = NHL.condensed_outcomes

scatter_plot = make_scatter(condensed_outcomes['alt'], condensed_outcomes['win_percent'], condensed_outcomes['images']) 
